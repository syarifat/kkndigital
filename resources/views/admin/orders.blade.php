    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
            <div>
                ...
                <p class="text-stone-400 text-sm mt-1">Login sebagai <strong class="text-stone-600">{{ session('admin_username') }}</strong></p>
            </div>
            <div class="flex items-center gap-3">
                ...
                <form action="{{ route('admin.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="py-2.5 px-4 border border-stone-300 hover:bg-red-50 hover:border-red-300 text-stone-600 hover:text-red-700 font-bold rounded-xl text-sm transition-all">
                        Keluar
                    </button>
                </form>
            </div>
                        ...
                        <tr class="hover:bg-stone-50 transition-colors">
                            ...
                            <td class="px-5 py-4">
                                <span class="inline-block text-[10px] font-black uppercase tracking-widest px-2.5 py-1 rounded-lg
                                    @if($order->package_name == 'basic') bg-stone-100 text-stone-600
                                    @elseif($order->package_name == 'standard') bg-blue-100 text-blue-700
                                    @elseif($order->package_name == 'pro') bg-green-100 text-green-700
                                    @elseif($order->package_name == 'premium') bg-amber-100 text-amber-700
                                    @endif">
                                    {{ $order->package_name }}
                                </span>
